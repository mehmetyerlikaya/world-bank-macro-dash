/**
 * Shared logic for fetching and building ApiResponse for a single country.
 * Used by both /api/worldbank and /api/worldbank/compare.
 */
...
/** Fetch and compute full ApiResponse for a single country. */
export async function fetchCountryData(
  ...
  for (const { indicator, values } of rawData) {
    ...
    normalizedMap.set(indicator, normalizeIndicator(indicator, label, values));
  }
  ...
  const buckets: ApiBucket[] = BUCKETS.map((bucket) => {
    ...
    if (!result) {
      return { id: bucket.id, name: bucket.name, score: 0, indicators: [] };
    }
    return {
      id: bucket.id,
      name: bucket.name,
  ...
  const allYears = indicators.flatMap((i) => (i.latestYear ? [i.latestYear] : [])).filter(Boolean);
  const latestYear =
    allYears.length > 0
      ? allYears.reduce((a, b) => (parseInt(a, 10) > parseInt(b, 10) ? a : b))
      : to;

  const valuesByIndicator = new Map(rawData.map((r) => [r.indicator, r.values]));

  const yearsSet = new Set<string>();
  rawData.forEach((r) => r.values.forEach((v) => yearsSet.add(v.year)));
  const sortedYears = [...yearsSet].sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
  ...
  DIFF_CHART_INDICATORS.forEach(({ code }) => {
    ...
    for (let i = 1; i < vals.length; i++) {
      diffs.push({ year: vals[i].year, diff: vals[i].value - vals[i - 1].value });
    }
    diffsByIndicator.set(code, diffs);
  });

  const diffYearsSet = new Set<string>();
  diffsByIndicator.forEach((diffs) => diffs.forEach((d) => diffYearsSet.add(d.year)));
  const sortedDiffYears = [...diffYearsSet].sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
