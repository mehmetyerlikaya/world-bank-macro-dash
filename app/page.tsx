    ...
    <main className="min-h-screen pt-14">
      <div className="max-w-3xl mx-auto px-6 sm:px-8 pt-16 sm:pt-24 pb-32">
        ...
        <header className="mb-20 animate-fade-in-up">
          <p className="text-ledger-accent text-sm font-semibold tracking-wider uppercase mb-4 animate-fade-in-up" style={{ animationDelay: "0ms", animationFillMode: "both" }}>
          ...
          <p className="text-ledger-ink-muted text-[15px] leading-relaxed mb-8 max-w-xl animate-fade-in-up" style={{ animationDelay: "240ms", animationFillMode: "both" }}>
        ...
        <section className="mb-20" aria-labelledby="sections">
          <h2 id="sections" className="font-display text-2xl font-bold text-ledger-ink mb-2 animate-fade-in-up" style={{ animationDelay: "50ms", animationFillMode: "both" }}>
            What’s in the report
          </h2>
          <p className="text-ledger-ink-muted mb-10 animate-fade-in-up" style={{ animationDelay: "100ms", animationFillMode: "both" }}>
          ...
          <div className="space-y-8">
            {SECTIONS.map((section, i) => (
              <article
                key={section.id}
                className="border-l-2 border-ledger-border pl-6 py-2 animate-fade-in-up hover:border-ledger-accent/50 transition-colors duration-300"
                style={{ animationDelay: `${150 + i * 80}ms`, animationFillMode: "both" }}
              >
                <p className="text-ledger-accent text-xs font-semibold uppercase tracking-wider mb-1">
                  {section.term}
                </p>
                <h3 className="font-display text-xl font-bold text-ledger-ink mb-2">
                  {section.title}
                </h3>
                <p className="text-ledger-ink-muted text-[15px] leading-relaxed">
                  {section.description}
                </p>
              </article>
            ))}
        ...
        {/* Key concepts */}
        <section className="mb-20 py-10 border-t-2 border-ledger-border" aria-labelledby="concepts">
          <h2 id="concepts" className="font-display text-2xl font-bold text-ledger-ink mb-6 animate-fade-in-up" style={{ animationDelay: "50ms", animationFillMode: "both" }}>
            Behind the numbers
          </h2>
          <dl className="space-y-6">
            {CONCEPTS.map((c, i) => (
              ...
              >
                <dt className="font-semibold text-ledger-ink mb-1">{c.term}</dt>
                <dd className="text-ledger-ink-muted text-[15px] leading-relaxed">{c.desc}</dd>
              </div>
            ))}
          </dl>
        </section>
        ...
        {/* CTA */}
        <div className="text-center py-12 border-t-2 border-ledger-border animate-fade-in-up">
          <Link
            href="/report"
